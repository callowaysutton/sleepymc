...
func (proxy *Proxy) Process() process.Process {
	...
	if proxy.Config.Pterodactyl.isPtero() {
		pterodactyl, err := process.NewPtero(proxy.Config.Pterodactyl.Endpoint, proxy.Config.Pterodactyl.Key, proxy.Config.Pterodactyl.ServerUUID)
		if err != nil {
			log.Println("Failed to create a Pterodactyl process; error:", err)
			return nil
		}
		proxy.Config.process = pterodactyl
		return pterodactyl
	}
...
func (proxy *Proxy) ContainerTimeout() time.Duration {
	proxy.Config.RLock()
	defer proxy.Config.RUnlock()
	return time.Millisecond * time.Duration(proxy.Config.ContainerTimeout)
}
...
func (proxy *Proxy) timeoutProcess() {
	...
	if proxy.ContainerTimeout() <= 60000 {
		return
	}

	proxy.cancelProcessTimeout()

	log.Printf("[i] Starting container timeout %s on %s", proxy.ContainerTimeout(), proxy.UID())
	timer := time.AfterFunc(proxy.ContainerTimeout(), func() {
